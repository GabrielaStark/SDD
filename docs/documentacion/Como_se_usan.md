...
Antes de empezar
Setup del proyecto
Los tres pipelines en una imagen
Fase 0: Elegir pipeline (greenfield / brownfield-rewrite / mantenimiento)
Fase 1: Levantamiento de requerimientos
5A. Greenfield
5B. Brownfield-rewrite
5C. Mantenimiento (sistema en prod + feature nuevo)
Fase 1.5 (opcional): Prototipo visual
Fase 2: Diseño técnico
Fase 3: Descomposición en tareas
Fase 4: Ejecución del código
Reglas de oro durante el uso
Troubleshooting
Glosario rápido
...
Importante para mantenimiento: si vas a usar el pipeline de mantenimiento, en lugar de clonar el framework como repo nuevo, lo que harás es copiar .claude/ + templates/ + las carpetas docs/ relevantes al repo del sistema en producción donde quieres meter el feature. Detalles en Fase 0.
...
Si NO aparecen los 8: revisa la sección Troubleshooting.
...
Greenfield → Fase 1, sección 5A
Brownfield-rewrite → Fase 1, sección 5B
Mantenimiento → Fase 1, sección 5C
...
Pipeline para agregar un feature a un sistema en producción sin romper lo que ya funciona. Si todavía no lo tienes claro, vuelve a Fase 0 y verifica.
...
Salir del pipeline de mantenimiento y planear un brownfield-rewrite del módulo afectado antes del feature. Después del rewrite, volver a mantenimiento para el feature.
Reducir scope del feature para evitar el cambio arquitectónico.
Aceptar el cambio arquitectónico y documentarlo explícitamente con un ADR especial en el design (eso ya es mantenimiento con riesgo elevado — el humano decide).
...
Le muestras al cliente la URL desplegada con las credenciales.
El cliente da feedback (verbal, por Slack, Loom, lo que sea).
Tú transcribes el feedback en docs/prototype/validation-log-v1.md:
...
El agente te avisa: "Esto es un cambio estructural, necesita pasar por analista-entrevistas."
Tú invocas al analista-entrevistas para actualizar requirements.md.
Apruebas el requirements actualizado (gate humano normal).
Vuelves a invocar al prototipador-visual con el contexto enriquecido.
...
Overview
Architecture (con diagrama Mermaid)
Data Model
Interface Contracts
Technical Decisions (ADRs)
Critical Flows (diagramas de secuencia Mermaid)
Error & Edge Case Strategy
Testing Strategy
Traceability (tabla obligatoria)
Mantenimiento (docs/features/<slug>/design.md) — mismas 9 secciones pero acotadas al delta:

Overview del Delta
Architecture (2.1 Arquitectura Heredada inmutable + 2.2 Delta Architecture con Mermaid diferenciando nuevo/modificado/existente)
Data Model Delta (solo entidades nuevas o modificaciones, ALTER explícito si modifica)
Interface Contracts Delta (endpoints nuevos + extensiones a existentes con qué preservan)
Technical Decisions (ADRs del delta, prefijo D001) + sección "Decisiones heredadas que se mantienen"
Critical Flows Afectados + sección "Coexistencia con flujos existentes"
Error & Edge Case Strategy del Delta
Testing Strategy (delta + tabla obligatoria invariantes → tests de regresión)
Traceability (tabla doble: criterios EARS del delta + invariantes preservadas)
Paso 5: Cómo validar
Construcción:
...
Lee design.md y requirements.md completos.
Te lista cuántas tareas estima generar (aproximado).
Mapea cada criterio EARS a tarea(s) tentativa(s).
Genera el tasks.md por capas arquitectónicas: Setup → Data Model → Data Access → Business Logic → API → UI → Integration Tests → Documentation.
Cada tarea tiene checkbox, número, verbo concreto, sub-pasos, criterio de hecho, y footer _Requirements: X.Y_.
Hace una pasada de podado (quita redundantes, parte las grandes, promueve tests a tareas independientes).
Auto-valida.
Paso 2 (variante): Qué esperar (mantenimiento)
El agente descompositor-riesgo-mantenimiento:

Lee design.md delta y requirements.md delta completos.
Mapea estructura de tests existentes (con Glob/Read).
Te lista cantidad estimada de tareas, cuántas invariantes a blindar, cuántos puntos de integración aislados.
Genera el tasks.md con estructura por riesgo:
...
Documentation
Cada tarea con footer doble: _Requirements: X.Y_ | _Invariants: I.A_.
Hace pasada de podado y auto-valida.
Paso 3: Cómo validar
Construcción:
...
Dale este prompt (cambia los números por los de tu lote):
...
Tres líneas de ese prompt garantizan la corrección — no las quites por acortar:
...
Tú revisas el resultado de cada tarea del lote: corres los tests, lees el código, verificas el criterio de hecho.

Marcas [x] solo las que pasen. Cierras la conversación. Abres otra para el siguiente lote.
...
Contexto ya cargado. Ejecuta las tareas 21-25 (Business Logic (servicios, clientes externos, validadores)) de corrido. No reinspecciones la estructura
del proyecto entre cada una.
...
Una fase a la vez. Un gate humano entre cada una. No avances con dudas.
No le dejes al agente rellenar huecos. Si pregunta, responde. Mejor 10 preguntas hoy que 100 bugs mañana.
Tú revisas, no el agente. La auto-validación del agente es el piso, no el techo. Tu revisión humana es la que decide.
Trabaja por lotes, nunca "ejecuta todo". El error fatal es soltar "lee tasks.md y ejecuta todo" sin revisión. Lo correcto es juntar tareas de la misma capa en una conversación y cargar el contexto una sola vez para ahorrar tokens (ver Fase 4 → El patrón recomendado: trabajar por lotes). Revisas tú al cerrar cada lote, y las tareas de validación van siempre solas.
Si una regla del SKILL no te encaja, el caso probablemente no es para SDD. No inventes excepciones — las reglas son absolutas a propósito.
Versiona todo. requirements.md, design.md, tasks.md viven en git. Cambios después de aprobar = commit nuevo, no edición silenciosa.
Adicionales para mantenimiento
Antes del primer feature, corre onboarding y reglas-negocio (skills auxiliares incluidas en el framework). Generan el sustrato CLAUDE.md + BIG_PICTURE.md + REGLAS_DE_NEGOCIO.md. Sin sustrato, el análisis pierde mucha calidad.
Surface of Contact e Invariantes Preservadas son el contrato del feature. Si las apruebas sin leer línea por línea, no te quejes después de regresiones.
Regression Shield primero, sin excepción. Los tests de blindaje van antes que el código nuevo. Saltarlos garantiza regresiones.
No-Regression Validation no es opcional. La última tarea del tasks.md es obligatoria. "Se ve bien" no es validación — correr la suite + verificar invariantes manualmente sí.
Si el feature requiere cambiar arquitectura, sal del pipeline. Eso ya no es mantenimiento. Decide: rewrite del módulo afectado, reducir scope, o aceptar el cambio con ADR especial.
11. Troubleshooting
...
Reescritura previa del módulo afectado: salir del pipeline de mantenimiento, hacer un ciclo SDD completo de brownfield-rewrite para ese módulo, después volver a mantenimiento para el feature.
Reducir el scope del feature: limitar lo que el feature hace para evitar el cambio arquitectónico.
Aceptar el cambio arquitectónico y documentarlo con un ADR especial en el design.md delta marcado como alerta crítica.
...
¿Qué tarea(s) tocaron el módulo donde se rompió la invariante?
¿Esas tareas tenían footer _Invariants: I.X_ correctamente?
¿Existía un test de blindaje (Blindar I.X) que debió detectar el problema antes? ¿pasó cuando se escribió?
