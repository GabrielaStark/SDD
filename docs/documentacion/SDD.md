...
6. [Anti-patrones que no debés cometer](#6-anti-patrones-que-no-debés-cometer)
7. [FAQ](#7-faq)
...
> En vez de pedirle código a un agente IA y descubrir qué construyó después, **escribís especificaciones formales primero** y el agente genera código a partir de ellas. Las specs son el artefacto primario, versionado en el repo, y el código es consecuencia.
...
**No hay "ejecutá todo el pipeline solo"**. Lo intentaste, falló, no insistas.
...
- **Greenfield**: vas a construir algo desde cero. Tenés material de descubrimiento (entrevistas, transcripciones, formularios, imágenes). → Vas a usar `analista-entrevistas`.
- **Brownfield**: hay un sistema legacy existente que querés reescribir o modernizar. Tenés acceso al código y posiblemente análisis previo. → Vas a usar `arqueologo-codigo`.

### Paso 2: Cargar inputs

**Si es greenfield**: meté en `docs/inputs/` todo el material crudo del levantamiento:
...
**Si es brownfield**: meté en `docs/analysis/` los `.md` con tu análisis arqueológico previo (notas de exploración, mapeo de módulos, observaciones). El código legacy debe estar accesible en el repo o como referencia.

### Paso 3: Invocar el subagente correspondiente

Abrí Claude Code en el repo y verificá que los agentes aparezcan:

```
/agents
```

Deberías ver los 4 subagentes. Si no, revisá que la carpeta `.claude/agents/` esté en la raíz del repo.
...
**No le dejés rellenar huecos solo**. Si te hace 10 preguntas, respondé las 10. Es mejor que invente.

### Paso 5: Revisión final del requirements.md

Vos validás manualmente. Si está bien, aprobás explícitamente. Si no, pedís cambios.
...
Repetí el ciclo de iteración → auto-validación → aprobación humana.
...
Con `tasks.md` aprobado, ejecutás **una tarea por sesión**:

1. Leé la siguiente tarea pendiente.
...
4. Vos revisás el resultado.
5. Iterás hasta que esté bien.
6. Marcás `[x]` en `tasks.md`.
7. Conversación nueva para la siguiente tarea.

**Nunca** le des "ejecutá todo tasks.md de un golpe". Rompe garantizado.
...
**Cuándo usarlo**: cuando arrancás un feature/producto de cero y tenés material de descubrimiento (transcripciones, formularios, imágenes).
...
**Cuándo usarlo**: cuando vas a modernizar/reescribir un sistema legacy y necesitás documentar formalmente su comportamiento.
...
Las constituciones compartidas. Cada subagente carga el skill correspondiente y aplica sus reglas. Si querés cambiar las convenciones del framework (ej. cambiar a EARS en español), editás el SKILL y los subagentes lo respetan.

### Templates

Archivos en `templates/` con la estructura canónica y guía inline. Útiles si:

- Querés entender la estructura sin invocar al agente
- Querés escribir un documento a mano
- Querés referencia rápida sin abrir el SKILL completo

---

## 6. Anti-patrones que no debés cometer

### Del flujo

- ❌ Saltar gates humanos. "Ejecutá todo el pipeline" rompe garantizado.
...
- ❌ Errores narrativos ("el sistema devuelve un error apropiado"). Tipalos como enum.
...
Lo partís. Una feature = un trío `requirements/design/tasks`. Si el design pasa de 800 líneas o tasks pasa de 50 tareas, partir. Anti-patrón confirmado por Thoughtworks: spec gigante upfront + big-bang release.
...
Sí. Los SKILLs son la constitución, pero los podés versionar por proyecto si el cliente tiene convenciones específicas. La estructura del framework asume que cada repo cliente tiene su propia copia.
...
Porque el análisis de requirements/design vale la inversión. Si querés bajar costo en pruebas iniciales, podés cambiar a Sonnet editando el `model:` field de cada subagente.

### ¿Cómo verifico que un requirements.md está "bien"?

Cada skill tiene un checklist de auto-validación al final. El subagente lo ejecuta antes de cerrar. Vos podés ejecutarlo manualmente cuando revisás el output.
...
Si la #3 falla, agregá explícitamente en el system prompt del subagente: "Antes de cualquier acción, leé `.claude/skills/<name>/SKILL.md` y aplicá sus reglas".
...
Este framework te da el formato y el workflow. El quality gate y el rigor en la revisión los ponés vos.
