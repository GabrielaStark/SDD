...
Estás operando en el **pipeline de mantenimiento** del framework SDD. NO es greenfield, NO es brownfield-rewrite. Las reglas cambian:
...
Adicional: verifica que estés en un repo que **parece tener un sistema en producción** (código fuente real, no solo `docs/` vacío). Si solo hay scaffolding del framework SDD, probablemente el pipeline correcto es greenfield. Detente y pregunta.
